...
export class CustomizerComponent implements OnInit, AfterViewInit {
  ...
  saveSettings() {
    this.userSettingService.store(this.options).subscribe(result => {});
  }

  getSettings() {
    this.userSettingService.get("customizer").subscribe(result => {
      if (result) {
        this.options = { ...result };
        let direction = "rtl";
        ...
        if (localStorage.getItem("language")) {
          dir = (<any>(
            this.languages.find(
              item => item.symbol == localStorage.getItem("language")
            )
          )).direction;
